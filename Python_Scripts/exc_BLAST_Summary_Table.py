...
pattern = re.compile(r'>([A-Z0-9:-]+)\t[A-Za-z0-9-_.():{}[\]<>*,]+\t[0-9.]+\t[0-9]+\t[0-9]?\t[0-9]?\t[0-9]?\t[0-9]+\t[0-9]+\t[0-9]+\t[a-z0-9-.]+\t[0-9]+')
...
BLAST_Summary_Table = pd.concat([Temporary_Table2.reset_index(drop=True), Evalue_DF.reset_index(drop=True)], axis=1)
